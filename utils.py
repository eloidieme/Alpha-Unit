...
def get_pnl_stats(date, prev, portfolio_df, insts, idx, dfs):
    ...
    for inst in insts:
        ...
        if units != 0:
            ...
            day_pnl += inst_pnl
            nominal_ret += portfolio_df.loc[idx - 1,
                                            f"{inst} w"] * dfs[inst].loc[date, "ret"]
...
class Alpha():
    ...
    def compute_meta_info(self, trade_range):
        for inst in self.insts:
            ...
            self.dfs[inst]["ret"] = -1 + self.dfs[inst]["close"] / \
                self.dfs[inst]["close"].shift(1)
            sampled = self.dfs[inst]["close"] != self.dfs[inst]["close"].shift(
                1).bfill()
            eligible = sampled.rolling(5).apply(
    ...
    def run_simulation(self):
        ...
        for i in portfolio_df.index:
            ...
            if i != 0:
                ...
                day_pnl, capital_ret = get_pnl_stats(date=date, prev=date_prev, portfolio_df=portfolio_df,
                                                     insts=self.insts, idx=i, dfs=self.dfs)
            ...
            nominal_tot = 0
            for inst in eligibles:
                forecast = 1 if inst in alpha_long else (
                    -1 if inst in alpha_short else 0)
                dollar_allocation = portfolio_df.loc[i, "capital"] / (
                    len(alpha_long) + len(alpha_short))
                position = forecast * dollar_allocation / \
                    self.dfs[inst].loc[date, "close"]
                portfolio_df.loc[i, inst + " units"] = position
                nominal_tot += abs(position *
                                   self.dfs[inst].loc[date, "close"])
            ...
            portfolio_df.loc[i, "leverage"] = nominal_tot / \
